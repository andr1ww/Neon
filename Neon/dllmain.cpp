...
void InitNullsAndRetTrues() {
	if (Fortnite_Version == 0) NullFuncs.push_back(Memcury::Scanner::FindPattern("48 89 54 24 ? 48 89 4C 24 ? 55 53 57 48 8D 6C 24 ? 48 81 EC ? ? ? ? 8B 41 08 C1 E8 05").Get());
		...
		NullFuncs.push_back(Memcury::Scanner::FindStringRef("Widget Class %s - Running Initialize On Archetype, %s.").ScanFor(Fortnite_Version < 6.3 ? std::vector<uint8_t>{ 0x40, 0x55 } : std::vector<uint8_t>{ 0x48, 0x89, 0x5C }, false).Get());
	}
		...
		NullFuncs.push_back(Memcury::Scanner::FindStringRef("Widget Class %s - Running Initialize On Archetype, %s.").ScanFor(std::vector<uint8_t>{ 0x48, 0x89, 0x5C }, false).Get()); // for 12.41
		...
		if (std::floor(Fortnite_Version) == 17) NullFuncs.push_back(Memcury::Scanner::FindPattern("48 8B C4 48 89 70 08 48 89 78 10 55 41 54 41 55 41 56 41 57 48 8D 68 A1 48 81 EC ? ? ? ? 45 33 ED").Get());
	...
	if (Fortnite_Version >= 19.00) NullFuncs.push_back(Memcury::Scanner::FindPattern("48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8B EC 48 83 EC 50 4C 8B FA 48 8B F1 E8").Get());
		...
		auto sRef = Memcury::Scanner::FindStringRef("Changing GameSessionId from '%s' to '%s'");
		NullFuncs.push_back(sRef.ScanFor({ 0x40, 0x55 }, false, 0, 1, 2000).Get());
	}
	if (Fortnite_Version < 16.40) {
		auto Addr = Memcury::Scanner::FindStringRef("STAT_CollectGarbageInternal");
	...
	if (Fortnite_Version == 0) RetTrueFuncs.push_back(Memcury::Scanner::FindPattern("48 89 5C 24 ? 48 89 6C 24 ? 57 41 56 41 57 48 81 EC ? ? ? ? 48 8B 01 49 8B E9 45 0F B6 F8").Get());
		...
		if (std::floor(Fortnite_Version) == 17) RetTrueFuncs.push_back(Memcury::Scanner::FindPattern("48 8B C4 48 89 58 08 48 89 70 10 48 89 78 18 4C 89 60 20 55 41 56 41 57 48 8B EC 48 83 EC 60 4D 8B F9 41 8A F0 4C 8B F2 48 8B F9 45 32 E4").Get());
...
void Main() {
    ...
    *(bool*)(Finder->GIsClient() + 1) = true;
	...
	Runtime::Hook<&AFortGameModeAthena::StaticClass>(L"ReadyToStartMatch", AFortGameModeAthena::ReadyToStartMatch, ReadyToStartMatchOriginal);
	Runtime::Hook<&AFortGameModeAthena::StaticClass>(L"SpawnDefaultPawnFor", AFortGameModeAthena::SpawnDefaultPawnFor);
	Runtime::Exec<&AFortPlayerControllerAthena::StaticClass>(L"ServerAcknowledgePossession", AFortPlayerControllerAthena::ServerAcknowledgePossession);

	int InternalServerTryActivateAbilityIndex = 0;

	if (Engine_Version > 4.20)
	{
		static auto OnRep_ReplicatedAnimMontageFn = (UFunction*)TUObjectArray::FindObject("OnRep_ReplicatedAnimMontage");
	...
	public:
		...
		World, WorldName, nullptr
	};

	 UKismetSystemLibrary::GetDefaultObj()->Call<"ExecuteConsoleCommand">(&Params);
}
