...
namespace Addresses {
    uint64_t WorldNetMode = 0;
    uint64_t GIsClient = 0;
    uint64_t TickFlush = 0;
    uint64_t InitHost = 0;
    uint64_t PauseBeaconRequests = 0;
    uint64_t SetWorld = 0;
    uint64_t GetMaxTickRate = 0;
    uint64_t DispatchRequest = 0;
    uint64_t CreateNetDriver = 0;
    uint64_t ProcessEvent = 0;
    uint64_t ApplyCharacterCustomization = 0;
    uint64_t KickPlayer = 0;
    uint64_t InitListen = 0;
    uint64_t GameSessionPatch = 0;
    uint64_t GetWorldContextFromObject = 0;
    uint64_t CreateNetDriverLocal = 0;
    uint64_t CreateChannel = 0;
    uint64_t SendClientAdjustment = 0;
    uint64_t CallPreReplication = 0;
    uint64_t RemoveNetworkActor = 0;
    uint64_t DemoReplicateActor = 0;
    uint64_t WorldGetNetMode = 0;
    uint64_t RepDriverServerReplicateActors = 0;
    uint64_t StaticFindObject = 0;
    uint64_t InternalTryActivateAbility = 0;
    uint64_t ConstructSpec = 0;
    uint64_t GiveAbility = 0;
    uint64_t GiveAbilityAndActivateOnce = 0;
    uint64_t GetInterfaceAddress = 0;
}

void UFinder::Init() {
    Addresses::WorldNetMode = FindWorldNetMode();
    ...
    Addresses::GameSessionPatch = FindGameSessionPatch();
    Addresses::GetWorldContextFromObject = FindGetWorldContextFromObject();
    Addresses::CreateNetDriverLocal = FindCreateNetDriverLocal();
    ...
    Addresses::RemoveNetworkActor = FindRemoveNetworkActor();
    Addresses::DemoReplicateActor = FindDemoReplicateActor();
    Addresses::WorldGetNetMode = FindWorldGetNetMode();
    Addresses::RepDriverServerReplicateActors = FindRepDriverServerReplicateActors();
    Addresses::StaticFindObject = FindStaticFindObject();
    Addresses::InternalTryActivateAbility = FindInternalTryActivateAbility();
    Addresses::ConstructSpec = FindConstructSpec();
...
uint64_t UFinder::FindWorldNetMode()
{
    ...
    for (int i = 0; i < 400; i++)
    {
        if ((*(uint8_t*)(uint8_t*)(BeginningFunction + i) == 0xE8) && (*(uint8_t*)(uint8_t*)(BeginningFunction + i - 1) != 0x8B))
        {
...
uint64 UFinder::FindStaticFindObject()
{
    if (Engine_Version == 500)
    {
        ...
        if (!addr)
            addr = Memcury::Scanner::FindPattern("48 89 5C 24 ? 48 89 74 24 ? 4C 89 64 24 ? 55 41 55 41 57 48 8B EC 48 83 EC 60 45 8A E1 4C 8B E9 48 83 FA").Get();
...
uint64 UFinder::FindTickFlush()
{
    if (Engine_Version == 4.16)
        return Memcury::Scanner::FindPattern("4C 8B DC 55 53 56 57 49 8D AB ? ? ? ? 48 81 EC ? ? ? ? 41 0F 29 7B").Get();

    if (Engine_Version == 4.19)
        return Memcury::Scanner::FindPattern("4C 8B DC 55 49 8D AB ? ? ? ? 48 81 EC ? ? ? ? 45 0F 29 43 ? 45 0F 29 4B ? 48 8B 05 ? ? ? ? 48").Get();
    ...
    if (Engine_Version == 4.27)
    {
        ...
        if (!addr)
            addr = Memcury::Scanner::FindPattern("48 8B C4 48 89 58 18 55 56 57 41 54 41 55 41 56 41 57 48 8D A8 ? ? ? ? 48 81 EC ? ? ? ? 0F 29 70 B8 0F 29 78 A8 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 48 8B F9 48 89 4D 38 48 8D 4D 40").Get();
...
uint64 UFinder::FindPauseBeaconRequests()
{
...
uint64 UFinder::FindGetMaxTickRate()
{
    if (SDK::Fortnite_Version >= 19.00) {
        return Memcury::Scanner::FindPattern(
...
uint64 UFinder::FindDispatchRequest()
{
    if (Fortnite_Version >= 23.00)
    {
        return Memcury::Scanner::FindPattern("48 8B C4 48 89 58 ? 48 89 70 ? 48 89 78 ? 55 41 56 41 57 48 8D 68 ? 48 81 EC ? ? ? ? 80 3D ? ? ? ? ? 48 8D 3D").Get();
    }
...
uint64 UFinder::FindCreateNetDriver()
{ 
...
uint64 UFinder::FindProcessEvent()
{
...
uint64 UFinder::FindApplyCharacterCustomization()
{
    Memcury::Scanner Scanner = Fortnite_Version >= 12.41 ? Memcury::Scanner::FindPattern("48 89 5C 24 ? 4C 89 44 24 ? 48 89 54 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 8D 6C 24 ? 48 81 EC ? ? ? ? 49 8B 00") : Memcury::Scanner::FindPattern("48 8B C4 48 89 50 ? 55 57 48 8D 68 ? 48 81 EC ? ? ? ? 80 B9");
...
uint64 UFinder::FindKickPlayer()
{
    ...
    if (Engine_Version >= 4.23 || Engine_Version <= 4.25)
...
uint64 UFinder::FindGameSessionPatch()
{
...
uint64 UFinder::FindGetWorldContextFromObject()
{
...
uint64 UFinder::FindCreateNetDriverLocal()
{
...
uint64 UFinder::FindCreateChannel()
{
...
uint64 UFinder::FindSendClientAdjustment()
{
    if (Fortnite_Version.GetMajorVersion() >= 20)
    {
        return Memcury::Scanner::FindPattern(
...
uint64 UFinder::FindCallPreReplication()
{
...
uint64 UFinder::FindRemoveNetworkActor()
{
    if (Fortnite_Version.GetMajorVersion() >= 20)
        return Memcury::Scanner::FindPattern(
...
uint64 UFinder::FindDemoReplicateActor()
{
    if (Fortnite_Version.GetMajorVersion() >= 20)
        return Memcury::Scanner::FindPattern(
...
uint64 UFinder::FindWorldGetNetMode()
{
    if (Fortnite_Version.GetMajorVersion() >= 20)
...
uint64 UFinder::FindRepDriverServerReplicateActors()
{
    if (Engine_Version == 4.20)
        return 0x53;
    else if (Engine_Version == 4.21)
        return Fortnite_Version.GetMajorVersion() == 5 ? 0x54 : 0x56;
    else if (Engine_Version >= 4.22 && Engine_Version <= 4.24)
        return Fortnite_Version >= 7.40 && Fortnite_Version < 8.40 ? 0x57 :
        ...
        return 0x5D;
    else if (Fortnite_Version.GetMajorVersion() == 14 || Fortnite_Version <= 15.2)
        return 0x5E;
    else if (Fortnite_Version >= 15.3 && Engine_Version < 5.00)
        return 0x5F;
...
uint64 UFinder::FindInternalTryActivateAbility()
{
    return Memcury::Scanner::FindStringRef(L"InternalTryActivateAbility called with invalid Handle! ASC: %s. AvatarActor: %s").ScanFor({ 0x4C, 0x89 }, false).Get();
}

uint64 UFinder::FindConstructSpec()
{
    ...
    {
        return Memcury::Scanner::FindPattern("80 61 29 F8 48 8B 44 24 ?").Get();
    }
}

uint64 UFinder::FindGiveAbilityAndActivateOnce()
{
    if (Engine_Version == 4.26)
    {
        ...
        if (!sig1)
            sig1 = Memcury::Scanner::FindPattern("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC 40 49 8B 40 10 49").Get();
...
uint64 UFinder::FindGetInterfaceAddress()
{
    if (Engine_Version <= 4.21)
        return Memcury::Scanner::FindPattern("48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC 20 33 FF 48 8B DA 48 8B F1 48").Get(); 
...
uint64 UFinder::FindGiveAbility()
{
