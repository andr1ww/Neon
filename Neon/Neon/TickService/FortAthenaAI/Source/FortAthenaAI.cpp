...
void TickService::FortAthenaAIService::SafeZonesPhase(FortAthenaAI& AI, float CurrentTime)
{
    ...
    static map<APawn*, AActor*> BestTargets;
    ...
    if (MovementDistanceSq < 10000.0f)
    {
        ...
        if (AI.StuckCounter >= 20)
        {
            ...
            AI.Controller->MoveToLocation(BotLocation + (ForwardVector * 1000.0f), 50.0f, true, false, false, true, nullptr, false);
            ...
            if (Inventory) {
                ...
                for (const auto& Entry : Items) {
                    if (!Entry || !Entry->GetItemEntry().GetItemDefinition())
                        continue;
                    auto* PickaxeDef = Cast<UAthenaPickaxeItemDefinition>(Entry->GetItemEntry().GetItemDefinition());
                    if (PickaxeDef) {
    ...
    if (AI.LastFrame % 2 == 0 && GameState->GetSafeZoneIndicator() && !GameState->GetSafeZoneIndicator()->GetNextCenter().IsZero()) 
    {
        FVector SafeZoneCenter = GameState->GetSafeZoneIndicator()->GetNextCenter();
        float SafeZoneRadius = GameState->GetSafeZoneIndicator()->GetNextRadius().Size();
        float DistanceToZone = UKismetMathLibrary::Vector_Distance(BotLocation, SafeZoneCenter);
        bool bInCurrentSafeZone = true;
        if (GameState->GetSafeZoneIndicator()->GetNextRadius().Size() > 0.0f) {
            float CurrentZoneDistance = UKismetMathLibrary::Vector_Distance(BotLocation, GameState->GetSafeZoneIndicator()->GetNextCenter());
            bInCurrentSafeZone = CurrentZoneDistance <= GameState->GetSafeZoneIndicator()->GetNextRadius().Size();
        }
        bool bInNextSafeZone = DistanceToZone <= SafeZoneRadius;
        bool bNeedUrgentMovement = false;
        if (!bInCurrentSafeZone || !bInNextSafeZone) {
            bNeedUrgentMovement = true;
        }
        else if (!bInNextSafeZone && DistanceToZone > SafeZoneRadius * 1.2f) {
            bNeedUrgentMovement = true;
        }
        if (bNeedUrgentMovement) {
            AI.Pawn->PawnStopFire(0);
            AI.Controller->MoveToLocation(SafeZoneCenter, 200.0f, true, false, false, true, nullptr, false);
            AI.Moving = true;
            FRotator SafeZoneRotation = UKismetMathLibrary::FindLookAtRotation(BotLocation, SafeZoneCenter);
            SafeZoneRotation.Pitch = 0.0f;
            AI.Controller->SetControlRotation(SafeZoneRotation);
            return; 
        }
        else {
            AI.Moving = false;
    ...
    if (AI.LastFrame % 5 == 0 && AI.NearestEnemy && AI.NearestEnemy->IsValidLowLevel() && NearestDistanceSq < 64000000.0f)
    {
        bool bHasLineOfSight = AI.Controller->LineOfSightTo(AI.NearestEnemy, BotLocation, true);
        if (bHasLineOfSight || NearestDistanceSq < 16000000.0f)
        {
            ...
            if (Distance < 500.0f || (!bHasWeapon && Distance < 1200.0f) || (CurrentTime - AI.LastFireTime > 3.0f && Distance < 1000.0f))
            {
                auto* Inventory = AI.Controller->GetInventory();
                if (Inventory) {
                    const auto& Items = Inventory->GetInventory().GetItemInstances();
            ...
            else if (bHasWeapon)
            {
                ...
                if (Distance < 2500.0f && CurrentTime - AI.LastFireTime > 0.25f)
                {
                    AI.Pawn->PawnStartFire(0);
                    AI.LastFireTime = CurrentTime;
                }
    ...
    if (AI.LastFrame % 7 == 0)
    {
        ...
        BestTargets[AI.Pawn] = BestTarget;
    }

    AActor* CurrentBestTarget = BestTargets.contains(AI.Pawn) ? BestTargets[AI.Pawn] : nullptr;
    ...
    else if (!AI.TargetLoot)
    {
        ...
        AI.Controller->MoveToLocation(WanderTarget, 100.0f, true, false, false, true, nullptr, false);
    }
}